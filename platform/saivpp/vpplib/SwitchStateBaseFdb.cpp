...
sai_status_t SwitchStateBase::addFdbEntry(
        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    return vpp_add_fdb_entry(serializedObjectId, switch_id, attr_count, attr_list);

}

sai_status_t SwitchStateBase::vpp_add_fdb_entry(
        _In_ const std::string &serializedObjectId,
        _In_ sai_object_id_t switch_id,
        _In_ uint32_t attr_count,
        _In_ const sai_attribute_t *attr_list)
{
    ...
    /* Extract the FDB entry from serializedObjectId for mac info*/
    ...
    auto attr_type = sai_metadata_get_attr_by_id(SAI_FDB_ENTRY_ATTR_BRIDGE_PORT_ID, attr_count, attr_list);
    if (attr_type == NULL)
    {
    ...
    sai_status_t status = get(SAI_OBJECT_TYPE_PORT, port_id, 1, &attr);
    ...
    if (vpp_get_hwif_name(object_id, bd_id, ifname) == true) 
    {
        const char *hwif_name = ifname.c_str();
        auto ret = l2fib_add_del(hwif_name, fdb_entry.mac_address, bd_id, is_add, is_static_mac);
        SWSS_LOG_NOTICE("FDB Entry Added Successfully");

    }
    else
    {
        SWSS_LOG_ERROR("FDB Entry: Invalid ObjectID for the hwif on this bridge");
        return SAI_STATUS_FAILURE;
    }
    return SAI_STATUS_SUCCESS;
}

sai_status_t SwitchStateBase::delFdbEntry(
        _In_ const std::string &serializedObjectId)
{
    SWSS_LOG_ENTER();

    vpp_del_fdb_entry(serializedObjectId);    
...
sai_status_t SwitchStateBase::vpp_del_fdb_entry(
        _In_ const std::string &serializedObjectId)
{
    ...
    {
        SWSS_LOG_NOTICE(" Invalid Attributes for Deleting FDB Entry");
        return SAI_STATUS_FAILURE;
    }
    ...
    /* To extract the port_id on which FDB Entry needs to be deleted */
    auto br_port_attrs = m_objectHash.at(SAI_OBJECT_TYPE_BRIDGE_PORT).at(sai_serialize_object_id(br_port_id));
    auto meta = sai_metadata_get_attr_metadata(SAI_OBJECT_TYPE_BRIDGE_PORT, SAI_BRIDGE_PORT_ATTR_PORT_ID);
    auto bp_attr = br_port_attrs[meta->attridname];
    ...
    {
        const char *hwif_name = ifname.c_str();
        SWSS_LOG_NOTICE(" delete FDB_ENTRY on hwif_name: %s ",hwif_name);
        l2fib_add_del(hwif_name, fdb_entry.mac_address, bd_id, is_add, is_static);
    }
    else
    {
        SWSS_LOG_ERROR("delete FDB: Invalid ObjectID for the hwif on this bridge");
...
sai_status_t SwitchStateBase::vpp_flush_fdb_entry(
                    _In_ sai_object_id_t switch_id,
                    _In_ uint32_t attr_count,
                    _In_ const sai_attribute_t *attr_list)
{
    ...
    for (uint32_t i = 0; i < attr_count; i++) {
        ...
        switch (attribute.id) 
        {
            ...
            default:
                SWSS_LOG_NOTICE(" Invalid FDB Flush Attributes for FDB_FLUSH ");
                return SAI_STATUS_FAILURE;
                break;
        }
    }
    switch (mode)
    {
        ...
        case 5: /*flush interface*/
            {
                ...
                {
                    const char *hwif_name = ifname.c_str();
                    SWSS_LOG_NOTICE(" FDB_FLUSH on interface hwif_name:  %s ",hwif_name);
                    l2fib_flush_int(hwif_name);
                }
                else
                {
        ...
        case 2:
        case 6: /*flush bd_id/vlan id*/
            {
                SWSS_LOG_NOTICE(" FDB_FLUSH on Bridge ID/ VLAN ID : %d ", bd_id);
        ...
        case 7: /*flush all*/
            {
                l2fib_flush_all();
                SWSS_LOG_NOTICE(" FDB_FLUSH on ALL bridge ports and VLAN Ids ");
            }
            break;

        default:
            SWSS_LOG_NOTICE(" Invalid Attributes for FDB_FLUSH %d", mode);
            return SAI_STATUS_FAILURE;
            break;
